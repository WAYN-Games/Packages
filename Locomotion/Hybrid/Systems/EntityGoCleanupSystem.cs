...
public partial struct EntityGoCleanupSystem : ISystem
{
    ...
    {
        EntityCommandBuffer ecbBis = SystemAPI.GetSingleton<BeginInitializationEntityCommandBufferSystem.Singleton>()
            .CreateCommandBuffer(state.WorldUnmanaged);
        foreach (var (presentationGoCleanup, entity)
                 in SystemAPI.Query<EntityGoCleanup>().WithAbsent<Simulate>().WithEntityAccess())
        {
            ...
            ecbBis.RemoveComponent<EntityGoCleanup>(entity);
        }
    }
}